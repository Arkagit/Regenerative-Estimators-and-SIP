...
norm_bmopt = matrix(0, nrow = length(reps), ncol = length(samp_size))
norm_bmth = matrix(0, nrow = length(reps), ncol = length(samp_size))
...
plot(log_10_n, colMeans(norm_bmopt), ylim = c(0.3, 0.5), type = 'l', col = "blue",
 ylab = "Frobenius Norm")
...
legend("topright", bty = "n",legend = c("BM(optimal)", "BM(Theory)"), 
	col = c("blue", "skyblue"), lty = 1, cex=0.5)

dev.off()

########################################################################
ess_bmopt = matrix(0, nrow = length(reps), ncol = length(samp_size))
ess_bmth = matrix(0, nrow = length(reps), ncol = length(samp_size))
...
plot(log_10_n, colMeans(ess_bmopt), type = 'l', ylim = c(0.07, 0.15), col = "blue",
	ylab = "ESS/n")
...
legend("topright", bty = "n",legend = c("BM(cuberoot)", "BM(sqroot)"), 
	col = c("blue", "skyblue"), lty = c(1,1), cex=0.5)
