...
plot(samp_size, colMeans(norm_bmopt), ylim = c(min(colMeans(norm_bmth), colMeans(norm_bmopt)), max(colMeans(norm_bmth), colMeans(norm_bmopt))), 
	type = 'l', col = "blue", ylab = "Frobenius Norm", xlab =  "Chain Length", main = "Frobenius Norm for Batch Means estimator")
segments(x0 = samp_size, y0 = colMeans(norm_bmopt) - 1.96*BMopt_frob, 
	y1 = colMeans(norm_bmopt) + 1.96*BMopt_frob, col = "blue", lty = 2)

lines(samp_size, colMeans(norm_bmth), type = 'l', col = "skyblue", lwd = 2)
segments(x0 = samp_size, y0 = colMeans(norm_bmth) - 1.96*BMth_frob, 
	y1 = colMeans(norm_bmth) + 1.96*BMth_frob, col = "skyblue", lty = 2, lwd = 2)
...
plot(samp_size, colMeans(ess_bmopt), type = 'l',  col = "blue", ylim = c(min(colMeans(ess_bmth), colMeans(ess_bmopt)), max(colMeans(ess_bmth), colMeans(ess_bmopt))),
	ylab = "ESS/n", xlab =  "Chain Length", main = "Effective Sample size of Batch Means estimator")
segments(x0 = samp_size, y0 = colMeans(ess_bmopt) - 1.96*BMopt_ess, 
	y1 = colMeans(ess_bmopt) + 1.96*BMopt_ess, col = "blue", lty = 2)

lines(samp_size, colMeans(ess_bmth), type = 'l', col = "skyblue", lwd = 2)
segments(x0 = samp_size, y0 = colMeans(ess_bmth) - 1.96*BMth_ess, 
	y1 = colMeans(ess_bmth) + 1.96*BMth_ess, col = "skyblue", lty = 2, lwd = 2)
